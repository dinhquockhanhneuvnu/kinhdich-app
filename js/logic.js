...
function duDoanThoiDiem(dt, nguyetKien, nhatThan, tuanKhong, haoVaiTro) {
  ...
  if (tuanKhong.includes(dungThanDC)) {
    if (dtVS.diem < -3) {
      goiY.push({ ly_do: '🕳️ Chân Không (Vô khí)', thoi_diem: 'Không có ứng kỳ', ghi_chu: 'DT quá yếu lại gặp Không ➔ vĩnh viễn không thực hiện được.' });
      return goiY; // Kết thúc sớm vì Chân Không là đại hung
    }
    goiY.push({
      ly_do: '🕳️ Tuần Không',
  ...
  const moAddr = NHAP_MO[haoHanh];
  if (nhatThan === moAddr || nguyetKien === moAddr || (dt.bienDC && dt.bienDC === moAddr)) {
    goiY.push({
      ly_do: '🔒 Nhập Mộ',
  ...
  if (LUC_XUNG[nguyetKien] === dungThanDC) {
    if (dtVS.diem < -3) {
       goiY.push({ ly_do: '💥 Chân Phá (Vô khí)', thoi_diem: 'Không có ứng kỳ', ghi_chu: 'DT quá yếu lại bị Nguyệt Phá ➔ việc đổ vỡ hoàn toàn.' });
       return goiY;
    }
    goiY.push({
      ly_do: '💥 Nguyệt Phá',
  ...
  if (isPhuc || (haoVaiTro && state.hao6.some(h => h.phucThan && h.phucThan.lucThan === dt.lucThan))) {
    let phiDC = '';
    if (isPhuc) phiDC = state.dungThanPhucData.phiHao.diaChi;
    else {
        const hPhuc = state.hao6.find(h => h.phucThan && h.phucThan.lucThan === dt.lucThan);
