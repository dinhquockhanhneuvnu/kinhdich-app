function sinhLuanNghia(dt, haoVaiTro, vs) {
  ...
  if (lucThanInfo) {
    ...
    if (yTuong) {
      doan.push(
        `${lucThanInfo.icon} ${lucThanTen} lâm DT — ${isCat ? "ý tượng Cát" : "ý tượng Hung"}: ${yTuong}.`,
      );
    }
  }
  ...
  nhanXet.forEach((nx) => {
    if (nx.includes("Nguyệt Sinh") || nx.includes("Nhật Sinh")) {
      ...
      doan.push(` DT Ám Động — ${TUONG_XUNG_Y_TUONG.amDong}.`);
  ...
  if (dt.laTuanKhong) {
    ...
    let tkText =
      "DT Tuần Không — sự việc chưa có thực chất, trong lòng bất an.";
  ...
  if (dt.laNhapMo) {
    ...
    if (loaiViec) {
      const moMap = {
        "tai-loc": "taiVan",
        "benh-tat": "benhTat",
        "hon-nhan-nam": "honNhan",
        "hon-nhan-nu": "honNhan",
        "kien-tung": "kienTung",
      };
      const moKey = moMap[loaiViec];
      if (moKey && MO_KHO_Y_TUONG[moKey])
        moText += ` ${MO_KHO_Y_TUONG[moKey]}.`;
    }
    doan.push(`🔒 ${moText}`);
  }
  ...
  nguyenThans.forEach((nt) => {
    ...
    if (tt.huuDung) {
      ...
      if (hdCheck && hdCheck.loai.includes("sinh"))
        text += `. Lại ${TUONG_SINH_Y_TUONG.hoiDau}`;
      doan.push(text + ".");
    } else if (tt.voLuc) {
      doan.push(
  ...
  const catHung =
    state.finalCatHung ||
    (vs.diem >= 2 ? "Cát" : vs.diem >= 0 ? "Bình" : "Hung");
  const finalScore =
    state.finalScore !== undefined ? state.finalScore : vs.diem;

  if (catHung.includes("Cát")) {
    doan.push(
      ` **Kết luận: ${catHung}** — Tổng lực lượng (bao gồm tương tác sinh khắc trong quẻ) đạt ${finalScore.toFixed(1)} điểm. Mưu sự thuận lợi, sự việc có khả năng thành công.`,
    );
  } else if (catHung.includes("Bình") || catHung.includes("Phục Tàng")) {
    doan.push(
      ` **Kết luận: ${catHung}** — Tổng lực lượng đạt ${finalScore.toFixed(1)} điểm. Dụng thần ở thế giằng co hoặc tiềm ẩn, kết quả còn chờ thời cơ, cần xem ứng kỳ.`,
    );
  } else {
    doan.push(
      ` **Kết luận: ${catHung}** — Tổng lực lượng bị đè nén/xung phá còn ${finalScore.toFixed(1)} điểm. Mưu sự khó thành, nhiều cản trở, cần hết sức thận trọng.`,
    );
  }
