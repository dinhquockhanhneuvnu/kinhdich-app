...
function convertToAmLich() {
  ...
  try {
    ...
    showToast('Lỗi chuyển đổi âm lịch. Vui lòng nhập Can Chi thủ công.', 'error');
  }
}
...
function luuQue() {
  ...
  showToast('✅ Đã lưu quẻ thành công!', 'success');
}
function copyFullPromptV3() {
  ...
  if (!dt) {
    showToast('Vui lòng chọn Dụng Thần trước để app tính toán lực lượng!', 'warning');
    return;
  }
...
- Lời khuyên cụ thể cho người gieo quẻ.`;
  ...
  navigator.clipboard.writeText(text).then(() => {
    showToast('✅ Đã Tạo Prompt & Copy vào Clipboard!', 'success');
  }).catch(err => {
